...
export const getTransactionsByMonth = createServerFn({
  ...
  .handler(async ({ context, data }) => {
    const earliestData = new Date(data.year, data.month - 1, 1);
    const latestData = new Date(data.year, data.month, 0);
      ...
      .where(
        and(
          ...
          gte(
            ...
            format(earliestData, "yyyy-MM-dd")
          ),
          lte(
            transactionsTable.transactionDate,
            format(latestData, "yyyy-MM-dd")
          )
        )
      )
